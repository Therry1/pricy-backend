...
from app.services.user_manager.helper import hash_password
...
async def get_users (db:AsyncSession ,user_id : UUID,  first_name : str):
    query = (
        select(UserAccount).where(UserAccount.deleted_at.is_(None))
    )
    try:
        if first_name :
            query.where(UserAccount.first_name == first_name)
        if user_id:
            query.where(UserAccount.first_name == first_name)
            
        users_account_stmt = await db.execute(query)
        users_accounts = users_account_stmt.scalars().all()
